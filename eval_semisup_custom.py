...
parser.add_argument("--data_norm_params", type=str, default="",
                    help="path to the norma parameters of the dataset")
...
def main():
    ...
    with open(f'{args.data_path}/train/subset_{args.labels_perc}_percent.txt', 'r+') as f:
        list_imgs = [line.replace("\n", "") for line in f.readlines()]

    train_dataset.samples = [(
        os.path.join(train_data_path, li),
        train_dataset.class_to_idx[li.split('/')[0]]
    ) for li in list_imgs]
    ...
    # build model
    model = resnet_models.__dict__[args.arch](output_dim=args.n_classes)
    ...
    if state_dict:
        ...
        logger.info("Load pretrained model ({}) with msg: {}".format(args.pretrained, msg))
    else:
    ...
    writer = SummaryWriter(log_dir=f"{args.dump_path}/run")
    for epoch in range(start_epoch, args.epochs):
        ...
        scores = train(model, optimizer, train_loader, epoch, writer)
        # save checkpoint
        if args.rank == 0:
            save_dict = {
                "epoch": epoch + 1,
            ...
            torch.save(save_dict, os.path.join(args.dump_checkpoints, "checkpoint-last.pth.tar"))
        scores_val = validate_network(val_loader, model, writer, epoch * len(train_loader))
...
def train(model, optimizer, loader, epoch, writer):
    ...
    for iter_epoch, (inp, target) in enumerate(loader):
        ...
        if args.rank == 0 and iter_epoch % 50 == 0:
            ...
            writer.add_scalar("loss/train", losses.val, iter)
            writer.add_scalar("top1/train", top1.val, iter)

    return epoch, losses.avg, top1.avg.item(), top5.avg.item()


def validate_network(val_loader, model, writer, iter):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    if top1.avg.item() > best_acc[0]:
        ...
        shutil.copyfile(
            os.path.join(args.dump_checkpoints, "checkpoint-last.pth.tar"),
            os.path.join(args.dump_checkpoints, "checkpoint-best.pth.tar"),
        )
    ...
    if args.rank == 0:
        ...
        writer.add_scalar("top1/val", top1.avg, iter)
